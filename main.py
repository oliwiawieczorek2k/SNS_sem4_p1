...
if __name__ == '__main__':
    ...
    file_name = "almanac.yuma.week0098.503808.txt"
    almanac = get_data(file_name)

    #DANE
    #zamina współrzędnych geodezyjnych fi i lambda, podanych w stopniach, na rady
    fi = numpy.deg2rad(52.0)
    lambd = numpy.deg2rad(21.0)
    h = 100.0

    #określenie wartości czasowych początkowych i końcowych przy zachowaniu załozenia,
    #że czas końcowy jest póżniejszy niż początkowy ale oba dotyczą tego samego dnia
    #obydwie daty są zamieniane na gps time
    date_start = gps_time([2021, 3, 1, 00, 00, 00])
    date_end = gps_time([2021, 3, 1, 23, 59, 59])

    #maska
    mask = 0

    #interwał czasowy podany w sekundach
    dt = 10*60

    #określenie satelitów w postaci [1] lub [1,2,3] lub "all"
    nr_sat = [1, 2, 30, 31, 32]

    #WYKRESY
    #elewacji od czasu
    plot_el(date_start,date_end,dt, nr_sat, mask)

    #liczba widocznych satelit od czasu
    nr_visible(date_start, date_end, dt, mask)

    #wartosc wspolczynnikow DOP od czasu
    plot_dop(date_start, date_end, dt, fi, lambd, h, mask)

    #skyplot
    plot_skyplot(date_start, fi, lambd, h, mask)
